export default class XrefModel {
    ...
    addResult(result) {
        ...
        while (ix < this.all_sub_results.length) {
            if (result.count > this.all_sub_results[ix].count) { break; }
            ix++;
        }
    ...
    prepareResult(result, root_result, depth) {
        ...
        result.identifier = root_result.identifier;
        result.depth = depth;
        // check the labels
    ...
    processFilteredResults(data_results, result, fromPaging) {
        ...
        result.lastFilteredPageKey = data_results[0].identifier; //this is magic for now.
    }
    ...
    processPagingResults(results, result_org) {
        ...
        for (let key in results) {
            ...
            for (let key2 in result.results) {
                let sub_result = result.results[key2];
                if (result.identifier === result_org.identifier && sub_result.domain_id === result_org.domain_id) {
