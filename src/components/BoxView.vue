...
<!-- 		        <a :href='xref_conf[""+sub_res.domain_id].url.replace("£{id}",sub_res.identifier)' target='_blank'><i class="fas fa-external-link-alt fa-1x"></i></a>  -->
			    ...
			    <a  class="actionIcon icon" title="Show results"  @click="hideResults(sub_res)" v-show="sub_res.showResults"><i class="fas fa-eye-slash"></i></a>
			    <a  class="actionIcon icon" title="Hide results"  @click="showResults(sub_res)" v-show="!sub_res.showResults"><i class="fas fa-eye"></i></a>
			    <a  class="actionIcon icon" title="Show selection list"  @click="sub_res.treeModal=true" v-show="!parent_sub_res"><i class="fas fa-list-ul"></i></a>
           ...
           <div class="allEntryBox">
			<div :id="'sub_res'+sub_res.counter" class="flex-container">
				<div v-for="entry in sub_res.displayEntries" :style="entry.style">
					...
					<a @click='selectXref(sub_res.identifier,sub_res.domain_id,entry.xref_id,entry.domain_id,sub_res,entry)' 
					   v-show="!entry.selected" ><i class="fas fa-plus-circle plusi"></i></a>
					</p> 
					<a class="exlinkcolor" :href='entry.url'
						   target='_blank' :title="entry.title"><small>{{ entry.label }}</small></a>
					</p>
					<p>	
...
<!-- 						   target='_blank'><i class="fas fa-external-link-alt fa-1x"></i></a> -->
			...
			<div class="has-text-centered pagingDiv" v-show="sub_res.maxClientPage > 0"> 
                ...
                <a title="Next Page" v-show="sub_res.maxClientPage > 0 && sub_res.clientPage<sub_res.maxClientPage" @click="nextPage(sub_res)"><i class="fas fa-arrow-right"></i></a>
			</div>
			...
			<div class="modal" v-bind:class="{ 'is-active' : sub_res.filterModalActive}">
			  <div class="modal-background" @click="sub_res.filterModalActive=false"></div>
			  <div class="modal-content box"> 
			       ...
			       <div class="flex-container2">
				       ...
				       <label class="checkbox">
						  <input type="checkbox" v-model="domain_count.selected" />
						   {{ domain_count.filterLabel }}
						</label>
				       </div>
			       </div>
				    <div class="buttons has-addons is-right">  
				       <button  :class="{ 'is-loading' : filterLoading,'button':true, 'is-warning':true, 'is-medium':true}" @click="applyFilter(sub_res)">Apply</button>
				    </div>   
			  </div>
			    	  <button class="modal-close is-large" aria-label="close" @click="sub_res.filterModalActive=false"></button>
			</div>
		</div>	
		...
		<div class="modal" v-bind:class="{ 'is-active' : sub_res.treeModal}">
		...
		<button class="modal-close is-large" aria-label="close" @click="sub_res.treeModal=false"></button>
		</div>
		</template>
 ...
 props: {
  ...
  methods:{
      ...
      selectXref: function(id,domain_id,entry_id,entry_domain_id,sub_res,entry){
          ...
          let callback_params= [id,domain_id,entry_id,entry_domain_id,sub_res,entry];
          this.$root.$data.fetcher.search(entry_id, this.processSelectedXref.bind(this),callback_params);
          
      },
      processFilteredResults: function (data_results,sub_result,callback_params,fail) {     
      ...
      applyFilter: function (sub_result) {
          ...
          let callback_params= [];
          this.$root.$data.fetcher.searchByFilter(sub_result,this.processFilteredResults.bind(this),callback_params);
          
      },
      processPaginResults: function (results,callback_params) { 
          this.$root.$data.model.processPaginResults(results,callback_params[0]);
          callback_params[0].displayEntries = callback_params[0].entries.slice(callback_params[1],callback_params[2]);
      },
      setDisplayEntries	(sub_result){
          
          let start = sub_result.clientPage*this.app_conf.page_size;
          let end = sub_result.clientPage*this.app_conf.page_size+this.app_conf.page_size;
          ...
          }else if(end<=sub_result.count){// fetch more data from server.
              if(sub_result.hasFilter){
                  let callback_params= [start,end];
                  this.$root.$data.fetcher.searchByFilter(sub_result,this.processFilteredResults4Paging.bind(this),callback_params);
              }else{
                  let callback_params= [sub_result,start,end];
                  this.$root.$data.fetcher.searchByPageIndex(sub_result.identifier,sub_result.domain_id,sub_result.serverPage,sub_result.maxServerPage,this.processPaginResults.bind(this),callback_params);      
              }
      ...
      removeXref(sub_res,parent_sub_res){
          ...
          for(let key in parent_sub_res.selectedXrefs){
              ...
              if(s.identifier === sub_res.identifier && s.domain_id === sub_res.domain_id){
          ...
          for(let key in parent_sub_res.entries){
              ...
              if(sub_res.identifier === entry.xref_id && entry.domain_id === sub_res.domain_id ){
